...
public class Ph : EzoBase
{
    public Ph()
    {
        ...
        this.WhenAnyValue(p => p.MaxValue).Subscribe(value => { _ = PublishMessageWithTypeAsync(PoolControlHelper.GetPropertyName(() => MaxValue), PoolControlHelper.format1Decimal(value), true); });
