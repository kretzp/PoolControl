...
public class Redox : EzoBase
{
    public Redox()
    {
        ...
        this.WhenAnyValue(r => r.Off).Subscribe(ein => { _ = PublishMessageWithTypeAsync(PoolControlHelper.GetPropertyName(() => Off), ein.ToString(), true); OnValueChange(); });
        this.WhenAnyValue(r => r.On).Subscribe(aus => { _ = PublishMessageWithTypeAsync(PoolControlHelper.GetPropertyName(() => On), aus.ToString(), true); OnValueChange(); });
    }
